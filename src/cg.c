...
LLVMValueRef gen_llvm_assign2(struct gen_context *ctx, LLVMValueRef dst, LLVMValueRef src, const struct type *dst_type, const struct type *src_type, unsigned src_array_size);
...
int sic_type_same(const type_t *a, const type_t *b)
{
    return (a == b);
}

/* Append named metadata TODO Not working properly */
void llvm_attach_metadata(struct gen_context *ctx, LLVMValueRef val, const char *txt)
{
    static unsigned int c = 0;
    char tmpbuf[1024];

    int ln = snprintf(tmpbuf, 1024, "tmpmeta_%u_%s", ++c, txt);
    tmpbuf[1023] = 0;
    LLVMMetadataRef mtref = LLVMMDStringInContext2(ctx->context_ref, tmpbuf, ln);
    LLVMSetMetadata(val, 0, LLVMMetadataAsValue(ctx->context_ref, mtref));
}
...
LLVMTypeRef llvm_type_wrap_array2(LLVMTypeRef res, unsigned arraysize)
{
    return LLVMArrayType(res, arraysize);
}

LLVMTypeRef llvm_type_wrap_array(LLVMTypeRef res, struct variable *v)
{
    if (v->array)
        return LLVMArrayType(res, v->array);
    return res;
}

LLVMTypeRef llvm_type_wrap(LLVMTypeRef ref, int ptr)
{
    FATAL(ptr < 0, "Pointer value is negative!");
    while (ptr) {
        ref = LLVMPointerType(ref, 0);
        ptr--;
    }
    return ref;
}
...
const struct type *sic_solve_type(struct gen_context *ctx, const struct type *a, const struct type *b)
{
    ...
    if (b->type == V_NULL)
        return a;
    if (a->type == V_VOID && a->ptr)
        return b;
    if (b->type == V_VOID && b->ptr)
        return a;
    ...
    const type_t *restype = __find_type_by(ctx, a->type, wanted_bits, wanted_sign, 0, NULL);
    if (!restype)
        restype = a;
    // FIXME
#if 0
    if (ptr)
        restype = type_wrap_to(ctx, restype, ptr);
#else
    (void)ptr;
#endif
    return restype;
    //return type_wrap_to(ctx, __find_type_by(ctx, a->type, wanted_bits, wanted_sign, 0, NULL), ptr);
}
...
struct variable *find_variable_incomplete(struct gen_context *ctx, int reg)
{
    ...
    if (ctx->parent)
        return find_variable_incomplete(ctx->parent, reg);
    return NULL;
}
...
variable_t *sic_new_int_lit(struct gen_context *ctx, literalnum value, int sign, int bits)
{
    ...
    struct variable *var = new_variable(
        ctx, NULL, V_INT, bits,
        sign ? TYPE_SIGNED : TYPE_UNSIGNED,
        0, 0, ctx->global);
    var->value = value;
    var->val = gen_llvm_int_const_ref(bits, value, sign);
    var->kind = VAR_LITERAL;
...
variable_t *sic_new_real_lit(struct gen_context *ctx, double value, int bits)
{
    ...
    var->val = gen_llvm_real_const_ref(bits, value);
    var->kind = VAR_LITERAL;
    return var;
}
...
struct variable *gen_func(struct gen_context *ctx, struct node *node)
{
    ...
    LLVMTypeRef ret_type = LLVMInt32TypeInContext(func_ctx->context_ref);
    LLVMValueRef retval = LLVMConstInt(ret_type, 0, 0);
    ...
    if (!func_ctx->last_is_ret) {
        ...
        case V_INT:
            if (func_ctx->last_val && func_ctx->last_val->val) {
                const struct type *ret_type = find_type_by_name(ctx, "int");
                retval = gen_llvm_cast_to(func_ctx, func_ctx->last_val, ret_type);
            }
...
LLVMValueRef sic_cast_llvm_type2(struct gen_context *ctx, LLVMValueRef vref, const struct type *src_type, const struct type *type)
{
    LLVMValueRef res = vref;

    switch (type->type) {
    case V_FLOAT:
        if (src_type->type == V_INT) {
            if (src_type->sign)
                return LLVMBuildSIToFP(ctx->build_ref, vref, sic_type_to_llvm_type(type), llvm_gen_name());
            else
                return LLVMBuildUIToFP(ctx->build_ref, vref, sic_type_to_llvm_type(type), llvm_gen_name());
        }
        return LLVMBuildFPCast(ctx->build_ref, vref, sic_type_to_llvm_type(type), llvm_gen_name());
    case V_INT:
        if (src_type->type == V_STR && type->ptr) {
            /* Strings are ptrs, so use it as-is */
            return vref;
        }
    default:
        stack_trace();
        ERR("Invalid cast: %s to %s", stype_str(src_type), stype_str(type));
    }
...
{
    return sic_cast_llvm_type2(ctx, var->val, var->type, type);
}

LLVMValueRef sic_cast_llvm_size2(struct gen_context *ctx, LLVMValueRef src, const struct type *src_type, const struct type *type)
{
    ...
    if (type->bits > src_type->bits) {
        if (type->type == V_INT) {
            if (src_type->sign)
                res = LLVMBuildSExtOrBitCast(ctx->build_ref, src,
                        sic_type_to_llvm_type(type), llvm_gen_name());
            else
                res = LLVMBuildZExtOrBitCast(ctx->build_ref, src,
                        sic_type_to_llvm_type(type), llvm_gen_name());
        } else {
            /* FIXME */
#if 0
...
{
    return sic_cast_llvm_size2(ctx, var->val, var->type, type);
}

LLVMValueRef gen_array_gep2(struct gen_context *ctx, LLVMValueRef val, LLVMTypeRef array_type)
{
    ...
    return LLVMBuildInBoundsGEP2(
        ...
        array_type,
        val,
        indexes, 2, llvm_gen_name());
}

LLVMValueRef gen_array_gep(struct gen_context *ctx, struct variable *var)
{
    if (!var->array)
        return var->val;

    return gen_array_gep2(ctx, var->val, llvm_type_wrap_array(sic_type_to_llvm_type(var->type), var));
}
...
LLVMValueRef gen_llvm_assign2(struct gen_context *ctx, LLVMValueRef dst, LLVMValueRef src, const struct type *dst_type, const struct type *src_type, unsigned src_arr_size)
{
    if (!src)
        ERR("Invalid source");

    /* TODO FIXME Null can be passed as-is for now */
    if (!LLVMIsNull(src)) {
        // FIXME fail
#if 0
        if (src_type->ptr) {
            src = sic_cast_load_pointer2(ctx, src, src_type, dst_type);
        }
        if (src_arr_size && dst_type->ptr) {
            src = gen_array_gep2(ctx, src, llvm_type_wrap_array2(sic_type_to_llvm_type(src_type), src_arr_size));
        }
        if (dst_type->type != src_type->type)
            src = sic_cast_llvm_type2(ctx, src, src_type, dst_type);
#endif

    // FIXME TODO
#if 0
    if (vara->type->bits != varb->type->bits)
...
int gen_llvm_assign(struct gen_context *ctx, struct node *node)
{
    ...
    struct variable *vara;

    if (ctx->is_decl)
        vara = find_variable_incomplete(ctx, a);
    else
        vara = find_variable(ctx, a);
    ...
    LLVMValueRef src = sic_access_lit(ctx, varb, vara->type);
    if (!src)
        ERR("Invalid source");
    LLVMValueRef dst = vara->val;

#if 0
    if (ctx->global)
        LLVMSetInitializer(dst, src);
    else
        (void)LLVMBuildStore(ctx->build_ref, src, dst);
#endif
#if 1
    LLVMValueRef assigncmd = gen_llvm_assign2(ctx, dst, src, vara->type, varb->type, varb->array);
#if 0
    if (assigncmd)
        llvm_attach_metadata(ctx, assigncmd, vara->name);
#else
    (void)assigncmd;
#endif
#endif
...
int gen_decl_begin(struct gen_context *ctx, struct node *node)
{
    int a;
...
void parse_func_call_params(struct gen_context *ctx, struct node *node, unsigned int param_cnt, LLVMValueRef *args)
{
    ...
    while (node) {
        if (node->node == A_LIST) {
            ...
            // TODO Conversions to proper types
            if (var->array)
                val = gen_array_gep(ctx, var);
...
#if 0
        // FIXME Void ptr
        } else if (t->type == V_VOID && t->ptr) {
            res = LLVMBuildPointerCast(ctx->build_ref,
                var->val,
                LLVMPointerType(LLVMVoidType(), 0),
                llvm_gen_name());
#if 0
            res = LLVMBuildAlloca(ctx->build_ref,
                LLVMPointerType(LLVMVoidType(), 0),
                llvm_gen_name());
#endif
        } else if (var->type->type == V_VOID && var->type->ptr) {
            res = LLVMBuildPointerCast(ctx->build_ref,
                var->val,
                LLVMPointerType(sic_type_to_llvm_type(t), 0),
                llvm_gen_name());
#endif
        } else
...
LLVMValueRef sic_cast_load_pointer2(struct gen_context *ctx, LLVMValueRef vref, const struct type *src_type, const struct type *type)
{
    ...
    if (ptr > type->ptr) {
        ...
        while (ptr > 0 && ptr > dst) {
            ...
            if (ptr > 0)
                srctyp = llvm_type_wrap(srctyp, ptr - 1);
...
LLVMValueRef sic_cast_load_pointer(struct gen_context *ctx, struct variable *var, const struct type *type)
{
    ...
    return sic_cast_load_pointer2(ctx, vref, var->type, type);
}

LLVMValueRef sic_cast(struct gen_context *ctx, variable_t *var, const type_t *type)
{
    LLVMValueRef vr_ref = gen_llvm_cast_to(ctx, var, type);
    const type_t *src_type = var->type;
...
LLVMValueRef sic_access_lit(struct gen_context *ctx, variable_t *var, const type_t *type)
{
    ...
    case VAR_LITERAL_BOOL:
        return sic_cast(ctx, var, type);
    case VAR_VAR:
        return sic_cast_load_pointer(ctx, var, type);
    default:
        FATAL(NULL, "Invalid from sic to llvm!");
    }
    return NULL;
}
...
int gen_llvm_eq(struct gen_context *ctx, struct node *node)
{
    ...
    if (vara->type->type != varb->type->type && vara->type->type != V_NULL && varb->type->type != V_NULL) {
        if (vara->type->type == V_VOID && vara->type->ptr > 0)
            goto skip;
        if (varb->type->type == V_VOID && varb->type->ptr > 0)
            goto skip;
        ERR("Not same type: %s != %s",
            stype_str(vara->type), stype_str(varb->type));
    }
skip:
    (void)node;
#endif
    ...
    FATAL(!restype, "Cannot eq %s: %s and %s",
        node_str(node),
        variable_str(vara),
        variable_str(varb));
...
int gen_llvm_op(struct gen_context *ctx, struct node *node, int op)
{
    ...
    const struct type *restype = sic_solve_type(ctx, vara->type, varb->type);
    FATAL(!restype, "Cannot op %s: %s and %s",
        node_str(node),
        variable_str(vara),
        variable_str(varb));
...
int gen_llvm_negate(struct gen_context *ctx, struct node *node)
{
    ...
    case V_FLOAT:
        ...
        break;
    default:
        ERR("Invalid type for negate: %s", stype_str(vara->type));
    }

    return res->reg;
}

int gen_llvm_tilde(struct gen_context *ctx, struct node *node)
{
    struct variable *res = NULL;
    int a;
    ...
    case V_INT:
        ...
        res->val = LLVMBuildXor(ctx->build_ref, neg1, va,
                llvm_gen_name());
        break;
    default:
        ERR("Invalid type for tilde op (~): %s", stype_str(vara->type));
    }

    return res->reg;
}

int gen_llvm_not(struct gen_context *ctx, struct node *node)
{
    struct variable *res = NULL;
    int a;
    ...
    case V_INT:
        if (is_ptr)
            ...
            zero = gen_llvm_int_const_ref(vara->type->bits, 0, 0);
            res->val = LLVMBuildICmp(ctx->build_ref, LLVMIntEQ, va, zero,
                    llvm_gen_name());
        }
        break;
    case V_FLOAT:
        if (is_ptr)
            ...
            zero = gen_llvm_real_const_ref(vara->type->bits, 0.0);
            res->val = LLVMBuildFCmp(ctx->build_ref, LLVMRealUEQ, va,
                    zero, llvm_gen_name());
        }
        break;
    default:
        ERR("Invalid type for NOT: %s", stype_str(vara->type));
    }
    //llvm_attach_metadata(ctx, res->val, vara->name);

    return res->reg;
}

int gen_llvm_addr(struct gen_context *ctx, struct node *node)
{
    int a;
...
int gen_llvm_index(struct gen_context *ctx, struct node *node)
{
    struct node *ident = node->left;
    struct variable *var = find_variable_by_name(ctx, ident->value_string);
    int b;
    ...
    FATAL(!varb, "No index found");

    LLVMValueRef idx = sic_cast_load_pointer2(ctx,
        varb->val,
        varb->type,
        find_type_by_name(ctx, "unsigned long")
        );
    ...
    FATAL(!var->array, "No arr");

    res->val = LLVMBuildInBoundsGEP2(
        ctx->build_ref,
        llvm_type_wrap_array(sic_type_to_llvm_type(var->type), var),
        var->val,
        indexes,
        2,
        llvm_gen_name()
...
int gen_llvm_memset(struct gen_context *ctx, struct variable *var)
{
    LLVMValueRef valp = gen_array_gep(ctx, var);
    LLVMValueRef val = LLVMBuildBitCast(
        ctx->build_ref,
        valp,
        LLVMPointerType(LLVMInt8Type(), 0),
        llvm_gen_name());
    LLVMBuildMemSet(ctx->build_ref,
        val,
        LLVMConstInt(LLVMInt8Type(), 0, 0),
        LLVMConstInt(LLVMInt64Type(), var->array * var->type->bits / 8, 0),
        0
        );
    return 0;
}
...
int gen_llvm_indexdef(struct gen_context *ctx, struct node *node)
{
    ...
    if (ctx->is_decl && node->right) {
        LLVMTypeRef arval_type;
        LLVMTypeRef artype;
        literalnum idx_val;
        struct variable *varb = NULL;

        arval_type = sic_type_to_llvm_type(ctx->pending_type);

        if (node->right->node == A_INT_LIT)
            idx_val = node->right->value;
        else {
            int b = gen_recurse(ctx, node->right);
            varb = find_variable(ctx, b);
            idx_val = varb->value;
        }
        var = new_variable_ext(ctx,
            ident->value_string,
            ctx->pending_type->type,
            ctx->pending_type->bits,
            ctx->pending_type->sign,
            ctx->pending_type->ptr,
            1, 0,
            ctx->pending_type->type_name);
        if (!idx_val && varb) {
            var->val = LLVMBuildArrayAlloca(ctx->build_ref,
                    arval_type,
                    varb->val,
                    var->name);
            var->array = 1;
        } else {
            FATAL(!idx_val, "Invalid array index value: %lld", idx_val);
            artype = LLVMArrayType(arval_type, idx_val);

            var->array = idx_val;
            var->val = LLVMBuildAlloca(ctx->build_ref, artype, var->name);
        }
        gen_llvm_memset(ctx, var);
    } else {
        ERR("FAIL indexdef");
    }

    return var ? var->reg : 0;
}
...
int gen_llvm_pre_post_op(struct gen_context *ctx, struct node *node)
{
    ...
    int is_ptr = sic_var_is_ptr(vara);
    if (vara->type->ptr)
        va = sic_cast_load_pointer(ctx, vara, type_unwrap(ctx, vara->type));
    else
    ...
    case A_POSTINC:
        if (is_ptr) {
            LLVMValueRef indexes[1] = { one };
            res->val = LLVMBuildInBoundsGEP2(
                ctx->build_ref,
                llvm_type_wrap_array(sic_type_to_llvm_type(res->type), res),
                va, indexes, 1, llvm_gen_name());
        } else if (vara->type->type == V_INT) {
...
int gen_llvm_op_assign(struct gen_context *ctx, struct node *node)
{
    ...
    //const struct type *restype = vara->type;
#if 1
    const struct type *restype = sic_solve_type(ctx, vara->type, varb->type);
    if (!restype) {
        restype = vara->type;
    ...
    int is_ptr = sic_var_is_ptr(vara);
    if (is_ptr) {
        if (node->node != A_ADD_ASSIGN && node->node != A_SUB_ASSIGN)
            ERR("Invalid operator for pointer: %s", node->value_string);
    ...
    if (is_ptr) {
#if 0
        LLVMValueRef idx = sic_cast_load_pointer(ctx, varb,
            find_type_by_name(ctx, "unsigned long"));
#endif
        // TODO Negate idx if needed
        //LLVMValueRef zero = LLVMConstInt(LLVMInt64Type(), 0, 0);
...
int gen_llvm_return(struct gen_context *ctx, struct node *node)
{
    ...
    LLVMValueRef va = sic_access_lit(ctx, var, ctx->return_type);
...
{
    return REG(sic_new_int_lit(ctx, n->value, n->value < 0 ? 1 : 0, n->bits));
#if 0
    int signext;
    int bits = n->bits;
...
{
    return REG(sic_new_real_lit(ctx, n->value + n->fraction, n->bits));
#if 0
...
int gen_llvm_string_const(struct gen_context *ctx, struct node *n)
{
    struct gen_context *glob = get_global_ctx(ctx);
    int slen = 1;
    char *tmpstr = convert_escape(n->value_string, &slen);
    struct variable *res = new_variable(glob, NULL, V_STR, slen, TYPE_UNSIGNED, 1, 0, 1);

    LLVMValueRef str1;
    LLVMTypeRef typeref = sic_type_to_llvm_type(res->type);

    str1 = LLVMBuildGlobalStringPtr(ctx->build_ref, tmpstr, llvm_gen_name());
    /* Handle string as u8* instead of arrays */
    // FIXME This is complaining in Verify, figure out why?
    res->val = LLVMBuildAlloca(ctx->build_ref, typeref, llvm_gen_name());
    LLVMBuildStore(ctx->build_ref, str1, res->val);
    free(tmpstr);
...
int gen_recurse(struct gen_context *ctx, struct node *node)
{
    ...
    case A_STR_LIT:
        return gen_llvm_string_const(ctx, node);
    case A_NULL:
    ...
    case A_GLUE:
        ...
        break;
    case A_INDEX:
        return gen_llvm_index(ctx, node);
    case A_INDEXDEF:
    ...
    case A_TILDE:
        return gen_llvm_tilde(ctx, node);
    case A_NOT:
